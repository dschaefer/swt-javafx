...
/**
 * This class provides a default drag under effect (eg. select, insert and
 * scroll) when a drag occurs over a <code>Table</code>.
 * 
 * <p>
 * Classes that wish to provide their own drag under effect for a
 * <code>Table</code> can extend the <code>TableDropTargetEffect</code> and
 * override any applicable methods in <code>TableDropTargetEffect</code> to
...
public class TableDropTargetEffect extends DropTargetEffect {
	...
	/**
	 * Creates a new <code>TableDropTargetEffect</code> to handle the drag under
	 * effect on the specified <code>Table</code>.
	 * 
	 * @param table
	 *            the <code>Table</code> over which the user positions the
